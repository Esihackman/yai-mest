          ...
          <div className="hidden md:flex items-center space-x-8">
            {['Home', 'About', 'Products', 'Showcase', 'Contact'].map((item) => (
              <button
                ...
                className="relative group py-2 text-gray-700 hover:text-blue-600 transition-colors duration-300"
              >
                <span>{item}</span>
          ...
          <div className="container mx-auto px-4 py-2 space-y-1">
            {['Home', 'About', 'Products', 'Showcase', 'Contact'].map((item) => (
              <button
                key={item}
                onClick={() => handleLinkClick(item.toLowerCase())}
                className="block w-full text-left px-4 py-3 text-gray-700 hover:bg-blue-50 hover:text-blue-600 rounded-lg transition-colors duration-300"
              >
                {item}
              </button>
            ))}
